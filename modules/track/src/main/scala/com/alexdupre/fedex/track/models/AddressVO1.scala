...
/** Address where the package was actually delivered. Contrast with destinationAddress, which is the location to which the package was
  ...
  *   included.<br> Example: ["1043 North Easy Street", "Suite 999"]. <br>Note: Street lines is shown in response only in secured flow. For
  *   non secured flow, street lines is not shown in the response.
  * @param city
  ...
  */
case class AddressVO1(
    addressClassification: Option[String] = None,
...
object AddressVO1 {
  ...
  given Encoder[AddressVO1] = new Encoder.AsObject[AddressVO1] {
    final def encodeObject(o: AddressVO1): JsonObject = {
      JsonObject.fromIterable(
        Vector(
          "addressClassification" -> o.addressClassification.asJson,
  ...
  given Decoder[AddressVO1] = (c: HCursor) => {
    for {
      addressClassification <- c.downField("addressClassification").as[Option[String]]
    ...
    } yield AddressVO1(addressClassification, residential, streetLines, city, stateOrProvinceCode, postalCode, countryCode, countryName)
  }
}
